...
namespace Knowledge.DBContext
{
    class DBKnowledge
    {
        ...
        public List<Definition> DefinitionSearch(int groupId, string search)
        {
            return db.Definitions.Where(v => (groupId == 0 || v.GroupId == groupId) && (search.Length == 0 || v.Code.Contains(search))).ToList();
        }
    ...
    int year, int pages, int languageId, decimal size, int formatId, string path, int typeId, string url)
        ...
        public List<Knowledge> KnowledgeList(int typeId, string title)
        {
            return (from t in db.Knowledges
                    where (typeId == 0 || t.TypeId == typeId) && (title.Length == 0 || t.Title.Contains(title))
        ...
        public int KnowledgeInsert(int typeId, string title, string content, string url, bool isFile)
        {
            try
            {
                return db.Knowledges.Where(v => v.Title == title).First().Id;
            }
            catch (Exception) { }
            Knowledge obj = new Knowledge();
            obj.TypeId = typeId;
        ...
        #region Rule
        public List<Rule> RuleList(int typeId, string title)
        {
        ...
        public void RuleInsert(string[] sourceObj)
        {
            try
            {
                if (db.Rules.Where(v => v.Title == sourceObj[2]).First().Id > 0) return;
            }
            catch (Exception) { }
            try
            {
        ...
        public int RuleInsert(int typeId, string title, string content, string url, bool isFile)
        {
            try
            {
                return db.Rules.Where(v => v.Title == title).First().Id;
            }
            catch (Exception) { }
            Rule obj = new Rule();
            obj.TypeId = typeId;
        ...
        public void RuleEdit(int id, int typeId, string title, string content, string url, bool isFile)
        {
